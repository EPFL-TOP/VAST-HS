...
def vast_handler(doc: bokeh.document.Document) -> None:
    ...
    cds_labels_source      = bokeh.models.ColumnDataSource(data=dict(x=[], y=[], size=[], has_drug=[], drug=[], hs=[], has_hs=[]))
    cds_labels_source_supp = bokeh.models.ColumnDataSource(data=dict(x=[], y=[], size=[], has_drug=[], drug=[], hs=[], has_hs=[]))
    ...
    drug_concentration = bokeh.models.TextInput(title="Conc. (µMol/%)", value='', width=90)
    drug_fish_stage    = bokeh.models.TextInput(title="Fish Stage:", value="", width=90)
    drug_duration      = bokeh.models.TextInput(title="Duration (min):", value="", width=90)
    ...
    #___________________________________________________________________________________________
    def compute_state(has_drug, has_hs):
        if has_drug and has_hs:
            return "both"
        elif has_drug:
            return "drug"
        elif has_hs:
    ...
    def update_view(cds):
        ...
        states = []
        for d, h in zip(data["has_drug"], data["has_hs"]):
            states.append(compute_state(d, h))

        # IMPORTANT: assign a *new* list so Bokeh detects the change
        data["state"] = states
    ...
    def add_source_well(attr, old, new):
        ...
        cds_labels_source_supp.data = dict(x=x_supp, y=y_supp, size=[50*NZOOM_WELLS_SOURCE]*len(x_supp), 
                                           has_drug=[False]*len(x_supp), drug=['']*len(x_supp), 
    ...
    def display_drugs_source_wellplate():
        ...
        for well_pos in source_well_positions_supp:
            ...
            if len(drug) != 0 or len(hs) != 0:
                ...
                if len(hs) != 0:
                    has_hs_supp.append(True)
                else:
                    has_hs_supp.append(False)
    ...
    def display_drug_hs_name(attr, old, new):
        ...
        if len(drugs) == 0 and len(hs) == 0:
            ...
            items_html_drugs = "".join(
                ...
                for drug in drugs)
            items_html_drugs = f"<ol>{items_html_drugs}</ol>"
            ...
            <b style='color:green; font-size:18px;'>
                Drug(s) or Heatshock(s) in selected well {well_position[0][1]}{well_position[0][0]}:
            </b>
